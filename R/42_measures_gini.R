compute_gini_info <- function(out_a, out_q, ki_prob = 0.95) {
  ...
  out_gini <- compute_gini(out_a, out_q, NN, TT, GG, ki_upp, ki_low)
  return(out_gini)
}
compute_gini <- function(out_a, out_q, NN, TT, GG, ki_upp, ki_low) {
  if (is.null(GG)) {
    ...
    for (gg in seq_len(GG)) {
      for (nn in seq_len(NN)) {
        for (tt in seq_len(TT)) {
          ...
          out_gini[nn, tt, gg, c(2, 3)] <- quantile(vec_tmp, probs = c(ki_upp, ki_low))
        }
      }
    }
    new_rn <- sub("^a_", "", rownames(out_a))
