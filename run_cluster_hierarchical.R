...
spec_01 <-  list(
   ...
   hyperpriors = list(
    ...
    OmegaReg0Scale = 1000,
    nu0 = 1000, # 1000,
    Psi0 = diag(3) * 1000# diag(3) * 27, 57, 1000
)
out <- IIGpkg::Gibbs2_SM_SA_sampler(
      p_joint = 1, #p_joint,
      B_par = 0,
...
IIGpkg::save_Gibbs_plots(
  ...
  onlyY = FALSE,
  hierachPrior = TRUE)
# out_tmp <- readRDS("data/output/empirical/Estimates/cs1_pj1_Reg7_B0_Om1000_D0_OmD1000_A1_Psi1000_nu1000/mu_b0_0_Sigma_b0_100_alpha_b0_255_beta_b0_256_IO5000/cs1_pj1_B0_Om_D0_OmD1000_A1_Psi1000_nu1000mu_b0_0_Sigma_b0_100_alpha_b0_255_beta_b0_256_IO5000.rds")
# par(mfrow = c(2,2))
# range_taking <- 130000:150000
# cf_ids_01 <- c(1, 2, 4, 5)
# cf_ids_02 <- c(16, 17, 19, 20, 21, 25, 26, 28, 29)
# cf_ids <- c(cf_ids_01, cf_ids_02)
# # cf_ids <- c(19, 20)
# for (id in cf_ids) {
#   plot(out_tmp$B[id,1, range_taking], type = "l",
#        main = paste0("Common factor no:", id ))
# }
# out_tmp2 <- out_tmp
# range_replace <- 140000:150000
# range_taking  <- 130000:140000
# for (id in cf_ids) {
#   out_tmp2$B[id, 1, range_replace] <- out_tmp2$B[id, 1, range_taking]
#     plot(out_tmp2$B[id,1, range_taking], type = "l",
#        main = paste0("Common factor no:", id ))
# }
# saveRDS(out_tmp2, "data/output/empirical/Estimates/cs1_pj1_Reg7_B0_Om1000_D0_OmD1000_A1_Psi1000_nu1000/mu_b0_0_Sigma_b0_100_alpha_b0_255_beta_b0_256_IO5000/cs1_pj1_B0_Om_D0_OmD1000_A1_Psi1000_nu1000mu_b0_0_Sigma_b0_100_alpha_b0_255_beta_b0_256_IO5000_adj.rds")
# IIGpkg::save_Gibbs_plots(
#   Gibbs = out_tmp2, #out$Gibbs2_SM_SA
#   burnin = burnin,
#   path = file.path(plotPath, "stale2"),
#   nameMat = nameMat,
#   nameReg = nameRegList[[out_tmp2$initials$wRegSpec]],
#   njointfac = out_tmp2$initials$njointfac, # out$Gibbs2_SM_SA$initials$njointfac,
#   type = type,
#   predictionCI = TRUE,
#   onlyY = FALSE,
#   hierachPrior = TRUE)
