...
GibbsSSM_2_old <- function(itermax = 15000,identmax = 5000, npara, nreg, njointfac, yObs, c_, D, D0, B, Phi, Q , type = "allidio", initX, initP, initU, wRegSpec, wReg, uReg = NULL,
                           B0, Vhat, incObsOld = 100000, incObsNew = 100000, covScale, VhatDiagScale, VhatDiagScale_start = NULL, VdiagEst, alpha0, beta0, countryA, A, scaleA, diagA, Psi0, nu0, shape0, rate0, storePath = "none", fPost, sampleA, identification = T, storeUnit = 10000){
  ...
  while(iter <= itermax){
    ...
    for(i in 1:N){
      ...
      Omega1 <-  tryCatch({solve(selectR %*% invOmega1 %*% t(selectR))},
                          error = function(e){
                            #print(invOmega1)
                            if(storePath != "none"){
                              if(storeCount == 0){
