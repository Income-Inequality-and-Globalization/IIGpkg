...
GibbsSSM_2 <- function(itermax = 15000,identmax = 5000, npara, nreg, njointfac, yObs, c_, D, D0, B, Phi, Q , type = "allidio", initX, initP, initU, wRegSpec, wReg, uReg = NULL,
                       B0, mu_b0, Sigma_b0, Omega0, alpha_b0, beta_b0, selectR, selectC, Vhat, incObsOld = 100000, incObsNew = 100000, covScale, VhatDiagScale, VhatDiagScale_start = NULL, VdiagEst, alpha0, beta0, countryA, A, scaleA, diagA, Psi0, nu0, shape0, rate0, storePath = "none", fPost, sampleA, identification = T, storeUnit = 10000){
  ...
  ncoef <- sum(selectR)
  BD0STORE <- matrix(0, nrow = ncoef, ncol = itermax)
  Omega0STORE <- matrix(0, nrow = ncoef, ncol = itermax)
  if(nreg != 0){
    DSTORE <- array(0, dim = c(Nnpara,N * nreg,itermax)) # speichert die Koeffizienten der Makroregressor-Koeffizienten
    DiSTORE <- array(0, dim = c(npara, nreg, N))
  ...
  if(VdiagEst){Vstart <- initials$Vhat[1,1,1]}
  ...
  while(iter <= itermax){
    ...
    if(!ident_block){
       ...
       if(countryA){
         ...
         for(i in 1:N){
          ...
          VhatArray_A <- array(apply(VhatSqrt, 3, function(X){X %*% A %*% t(X)}), c(npara, npara, N * TT))
       }
    ...
    if(Simf){
      if(!ident_block){
        ...
        invisible(capture.output(KF <- tryCatch({RcppSMCkalman::kfMFPD(yObs = yObs, uReg = uReg, wReg = wReg,
  ...
  fSTORE[,,iter] <- fPost
    ...
    for(i in 1:N){
      ...
      beta1 <- Omega1 %*% (selectR %*% c(beta1_mid) + invOmega0 %*% selectR %*% c(B0[,,i], D0[,,i])) 
     ...
     if(!ident_block){
       ...
       if(type == "allidio"){
        ...
        BSTORE[,,iter] <- B
        B_i <- makeBi(npara = npara, N = N, p = p, p_joint = p_joint, B_stack = B, type = type)
        ...
        BD_0 <- mu_sampler(N = N, npara = npara, nreg = nreg, njointfac = njointfac, B_i = B_i, D_i = DiSTORE, invOmega0 = invOmega0, mu_b0 = mu_b0, Sigma_b0 = Sigma_b0, selectR = selectR, type = type)
        B0 <- BD_0$B0
        D0 <- BD_0$D0
        BD0STORE[, iter] <- selectR %*% c(B0[,,1], D0[,,1])
        
        Omega0 <- omega_sampler(N = N, B_i = B_i, D_i = DiSTORE, B0 = B0, D0 = D0, alpha_b0 = alpha_b0, beta_b0 = beta_b0, selectR = selectR)
        ...
        uSTORE[,, iter] <- u
      if(VdiagEst){
        #u <- yObs - apply(fPost,2,function(x){B %*% x})
        u <- yObs - B %*% fPost - D %*% wReg 
        if(VhatDiagScale){
  ...
  #close(pb)
  return(list(f = fSTORE, B = BSTORE, D = DSTORE, A = ASTORE, V = VSTORE, u = uSTORE, BD0STORE = BD0STORE, Omega0STORE = Omega0STORE, blockCount = blockCount, errorMsg = errorMsg, initials = initials))
...
utuSum <- function(uSplit,VhatSqrt, TT, N, npara){
  ...
  for(i in 1:N){
    ...
    utu <- apply(utildeSplit[[i]], 2, function(x){x %*% t(x)})
